...
object DatabaseModule {
    ...
    /**
     * Language, detected and corrected. Nullable with no default value in the entity, so a plain
     * ADD COLUMN is the whole migration — every existing row simply has no language yet, which is
     * the honest state until something reads a tag or a name.
     */
    private val MIGRATION_17_18 = object : Migration(17, 18) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL("ALTER TABLE `books` ADD COLUMN `language` TEXT")
            db.execSQL("ALTER TABLE `book_overrides` ADD COLUMN `language` TEXT")
        }
    }

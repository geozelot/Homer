...
fun PrivacySettingsScreen(
    ...
    SettingsScaffold(stringResource(R.string.set_privacy_title), onBack, modifier) {
        SettingsGroup(title = stringResource(R.string.set_privacy_device_header)) {
            SettingsSwitchRow(
        ...
        SettingsGroup(title = stringResource(R.string.set_privacy_server_header)) {
            SettingsSwitchRow(
        ...
        // The only feature that reaches a server other than the user's own — its own group, so it
        // cannot be mistaken for one of the settings about the user's own server.
        SettingsGroup(title = stringResource(R.string.set_privacy_online_header)) {
            SettingsSwitchRow(
